...
<div class="container">
    ...
    <table id="listagem" class="listagem">
        ...
        <tbody>
        <?php
        $query = $this->noticia->GetNoticia("principal");
            ...
            <tr <?= $disabled ?>>
                ...
                <td>
                    <?php if ($dados->status == 1) { ?>
                        <?= anchor("noticia/desativarNoticiaPrincipal/?id=$dados->id", $desativar, "title='Desativar' class='desativaRegistro'") ?> <?= anchor("noticia/excluirNoticiaPrincipal/?id=$dados->id", $trash, "title='Excluir' class='deletaRegistro'") ?></td>
                    <?php } else { ?>
                        <?= anchor("noticia/ativarNoticiaPrincipal/?id=$dados->id", $ativar, "title='Ativar' class='ativaRegistro'") ?> <?= anchor("noticia/excluirNoticiaPrincipal/?id=$dados->id", $trash, "title='Excluir' class='deletaRegistro'") ?></td>
                    <?php } ?>
            </tr>
        <?php } ?>
        </tbody>
